...
const PRIMARY_RED = "hsl(0,72%,55%)";
const PRIMARY_RED_RGBA = "rgba(220, 38, 38, ";
const BG = "#0f0303";
...
export function FunscriptWaveform({ script, videoRef, className, style }: FunscriptWaveformProps) {
  ...
  const drawBackground = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    grad.addColorStop(0, PRIMARY_RED_RGBA + "0.25)");
    grad.addColorStop(1, PRIMARY_RED_RGBA + "0.0)");
    ...
    ctx.strokeStyle = PRIMARY_RED;
    ctx.lineWidth = 1.5;
    ctx.shadowColor = PRIMARY_RED;
    ctx.shadowBlur = 6;
    ctx.stroke();
    ctx.shadowBlur = 0;
