...
class ControlPanelUI:
    ...
    def _render_run_control_tab(self):
        ...
        # Reset video/script data button
        imgui.spacing()
        with destructive_button_style():
            if imgui.button("Reset##ResetVideoButton", width=-1):
                imgui.open_popup("Confirm Reset Video##ResetVideoPopup")
        _tooltip_if_hovered("Unload the current video and clear all generated script data")

        if imgui.begin_popup_modal(
            "Confirm Reset Video##ResetVideoPopup", True, imgui.WINDOW_ALWAYS_AUTO_RESIZE
        )[0]:
            imgui.text(
                "This will unload the current video and clear all generated\n"
                "script data. Both timelines will be wiped.\n"
            ...
            with destructive_button_style():
                if imgui.button("Confirm Reset##RunConfirm", width=pw):
                    app.file_manager.close_video_action(clear_funscript_unconditionally=True)
                    imgui.close_current_popup()
            imgui.same_line()
            if imgui.button("Cancel##RunCancel", width=pw):
                imgui.close_current_popup()
            imgui.end_popup()
    ...
    def _render_post_processing_tab(self):
        ...
        # Reset script data button — always visible regardless of plugin availability
        with destructive_button_style():
            if imgui.button("Reset Script##ResetScriptButton", width=-1):
                imgui.open_popup("Confirm Reset Script##ResetScriptPopup")
        _tooltip_if_hovered("Clear all script data on both timelines and remove loaded funscript paths")

        if imgui.begin_popup_modal(
            "Confirm Reset Script##ResetScriptPopup", True, imgui.WINDOW_ALWAYS_AUTO_RESIZE
        )[0]:
            imgui.text(
                "This will clear all script data for both timelines and\n"
                "remove any loaded funscript file associations.\n"
                "This action cannot be undone."
            )
            imgui.spacing()
            avail_w = imgui.get_content_region_available_width()
            pw = (avail_w - imgui.get_style().item_spacing[0]) / 2.0
            with destructive_button_style():
                if imgui.button("Confirm Reset##PPConfirm", width=pw):
                    fm = app.file_manager
                    if app.processor and app.processor.tracker and app.processor.tracker.funscript:
                        fs_proc.clear_timeline_history_and_set_new_baseline(1, [], "Script Reset")
                        fs_proc.clear_timeline_history_and_set_new_baseline(2, [], "Script Reset")
                    fm.loaded_funscript_path = ""
                    fm.funscript_path = ""
                    app.app_state_ui.heatmap_dirty = True
                    app.app_state_ui.funscript_preview_dirty = True
                    app.project_manager.project_dirty = True
                    app.logger.info("Script data reset.", extra={"status_message": True})
                    imgui.close_current_popup()
            imgui.same_line()
            if imgui.button("Cancel##PPCancel", width=pw):
                imgui.close_current_popup()
            imgui.end_popup()
    ...
    def _render_advanced_tab(self):
        ...
        # Reset all project data button (clears video, script, and UI state)
        with destructive_button_style():
            if imgui.button("Reset All Project Data##ResetProjectDataButton", width=-1):
                imgui.open_popup("Confirm Reset Project##ResetProjectPopup")
        _tooltip_if_hovered("Clear the video, all script data, and UI state — equivalent to starting fresh")

        if imgui.begin_popup_modal(
            "Confirm Reset Project##ResetProjectPopup", True, imgui.WINDOW_ALWAYS_AUTO_RESIZE
        )[0]:
            imgui.text(
                "This will clear the video, all script data, and UI state.\n"
                "All timeline points will be wiped and the project will\n"
            ...
            pw = (avail_w - imgui.get_style().item_spacing[0]) / 2.0
            with destructive_button_style():
                if imgui.button("Confirm Reset##AdvConfirm", width=pw):
                    app.reset_project_state()
                    imgui.close_current_popup()
            imgui.same_line()
            if imgui.button("Cancel##AdvCancel", width=pw):
                imgui.close_current_popup()
            imgui.end_popup()
    ...
    def _render_ai_model_settings(self):
        ...
        is_any_process_active = is_batch_mode or is_analysis_running or is_live_tracking_running
...
# ------- Post-processing helpers -------
    ...
    def _render_automatic_post_processing_new(self, fs_proc):
        ...
        proc_tools_disabled = sp.full_analysis_active or (proc and proc.is_processing)
    ...
    def _render_range_selection(self, stage_proc, fs_proc, event_handlers):
        app = self.app
        disabled = stage_proc.full_analysis_active or (app.processor and app.processor.is_processing)
    ...
    def _render_funscript_processing_tools(self, fs_proc, event_handlers):
        ...
        sp = app.stage_processor
        proc = app.processor
        disabled = sp.full_analysis_active or (proc and proc.is_processing)
    ...
    def _render_device_control_content(self):
        ...
        # Disconnect All / Reset Devices button (always visible)
        imgui.spacing()
        imgui.separator()
        imgui.spacing()
        with destructive_button_style():
            if imgui.button("Disconnect All##DisconnectAllButton", width=-1):
                imgui.open_popup("Confirm Disconnect All##DisconnectAllPopup")
        if imgui.is_item_hovered():
            imgui.set_tooltip("Disconnect all connected devices and clear session state")

        if imgui.begin_popup_modal(
            "Confirm Disconnect All##DisconnectAllPopup", True, imgui.WINDOW_ALWAYS_AUTO_RESIZE
        )[0]:
            imgui.text(
                "This will disconnect all connected devices and clear\n"
                "the current session state.\n"
                "You can reconnect devices at any time."
            )
            imgui.spacing()
            avail_w = imgui.get_content_region_available_width()
            pw = (avail_w - imgui.get_style().item_spacing[0]) / 2.0
            with destructive_button_style():
                if imgui.button("Confirm Disconnect##DeviceDisconnectConfirm", width=pw):
                    if self.device_manager.is_connected():
                    ...
                    self._osr_scan_performed = False
                    self._buttplug_discovery_performed = False
                    self.app.logger.info("All devices disconnected and session cleared.", extra={"status_message": True})
                    imgui.close_current_popup()
            imgui.same_line()
            if imgui.button("Cancel##DeviceDisconnectCancel", width=pw):
                imgui.close_current_popup()
            imgui.end_popup()
    ...
    def _render_native_sync_tab(self):
        ...
        try:
            ...
            # Stop & Reset Session button (always visible)
            imgui.spacing()
            imgui.separator()
            imgui.spacing()
            with destructive_button_style():
                if imgui.button("Stop & Reset Session##ResetStreamerButton", width=-1):
                    imgui.open_popup("Confirm Reset Session##ResetStreamerPopup")
            if imgui.is_item_hovered():
                imgui.set_tooltip("Stop the streaming server and reset all session state including client tracking")

            if imgui.begin_popup_modal(
                "Confirm Reset Session##ResetStreamerPopup", True, imgui.WINDOW_ALWAYS_AUTO_RESIZE
            )[0]:
                imgui.text(
                    "This will stop the streaming server (if running) and\n"
                    "reset all session state including client tracking.\n"
                ...
                pw = (avail_w - imgui.get_style().item_spacing[0]) / 2.0
                with destructive_button_style():
                    if imgui.button("Confirm Reset##StreamerResetConfirm", width=pw):
                        stop_ok = True
                        if self._native_sync_manager is not None:
                            stop_ok = self._stop_native_sync()
                            if stop_ok:
                                self._native_sync_manager = None
                        self._prev_client_count = 0
                        self._native_sync_status_cache = None
                        ...
                        self.app.app_settings.set("show_video_feed", True)
                        if stop_ok:
                            self.app.logger.info("Streamer session stopped and reset.", extra={"status_message": True})
                        else:
                            self.app.logger.warning("Streamer stop failed — session state cleared, but server may still be running. Check logs.", extra={"status_message": True})
                        imgui.close_current_popup()
                imgui.same_line()
                if imgui.button("Cancel##StreamerResetCancel", width=pw):
                    imgui.close_current_popup()
                imgui.end_popup()
    ...
    def _stop_native_sync(self) -> bool:
        """Stop streamer servers. Returns True on success, False on failure."""
        try:
