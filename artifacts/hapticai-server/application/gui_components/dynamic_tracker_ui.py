...
from config.tracker_discovery import get_tracker_discovery, TrackerCategory, TrackerDisplayInfo, HIDDEN_TRACKER_NAMES
...
class DynamicTrackerUI:
    ...
    def get_simple_mode_trackers(self) -> Tuple[List[str], List[str]]:
        ...
        # Get live trackers only for simple mode, excluding examples and hidden trackers
        for name, info in all_trackers.items():
            if (info.category in [TrackerCategory.LIVE, TrackerCategory.LIVE_INTERVENTION] and
                ...
                "example" not in info.display_name.lower() and
                info.internal_name not in HIDDEN_TRACKER_NAMES):
    ...
    def get_batch_gui_compatible_trackers(self) -> Tuple[List[str], List[str]]:
        ...
        # Exclude example trackers and hidden trackers
        for name, info in all_trackers.items():
