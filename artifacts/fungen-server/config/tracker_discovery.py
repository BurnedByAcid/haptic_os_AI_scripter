...
class DynamicTrackerDiscovery:
    ...
    def get_gui_display_list(self) -> Tuple[List[str], List[str]]:
        ...
        # Filter out example trackers and hidden trackers
        all_trackers = [t for t in all_trackers if "example" not in t.internal_name.lower() and "example" not in t.display_name.lower() and t.internal_name not in HIDDEN_TRACKER_NAMES]
